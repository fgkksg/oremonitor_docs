...
- 翌日の日射量の予測値をDiscordやLINEに毎日自動で投稿
<!--
...
- 一部の機器データは設定変更に対応しています
    - 設定値の書き換えは次のようなリスクがあります
        - 業者しか変更できない設定項目を触ると機器の誤作動になりかねません（最悪、故障）
        - 変更前の設定値を忘れると元に戻せなくなります
    - ただし、リモコンや機器本体のボタン等でユーザーが操作できる設定項目は、書き換えを行っても安全と考えられます
...
- Googleスプレッドシートに書き込みを行う場合
    ...
    - **俺モニター**の「設定＞OAuth＞Google OAuth＞OAuth認証」にて「認証」ボタンを押して認証を完了させます

### OAuth認証について
- **俺モニター**で「OAuth認証」を実行するとWebブラウザに切り替わりますが、ブラウザの種類によっては認証プロセスが完了しない場合があります
    - Amazon Fire系タブレットの場合はChromeブラウザを使うことで認証プロセスが完了しますが、標準ではChromeブラウザがインストールされていないため、事前にChromeブラウザをインストールしておく必要があります
- **俺モニター**はGoogleへのアプリ申請を行っていないため次の制限があります
    - 認証の有効期限が7日間に制限されます
        - 定期報告の実行前に認証期限が来てしまう場合はその都度、認証期限前後いずれかのタイミングで「設定＞OAuth＞Google OAuth＞OAuth認証」にて解除を行ってから再認証を行う必要があります
