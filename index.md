...
1. （俺モニター作者の作業）端末情報を追記した俺モニターを作り、更新バージョンとしてアップロードします
...
>[!WARNING]
>機器データへの書き込み（設定変更）は次のようなリスクがあります
>- リモコンや機器のボタン等では操作できない設定項目を触ると機器の誤作動になりかねません（最悪、故障）
>- 変更前の設定値を忘れると元に戻せなくなります
>- リモコンや機器のボタン等でユーザーが操作できる項目は、書き換えを行っても安全と考えられます
...
>[!NOTE]
>この認証方法は**Googleサービスアカウント**よりも手軽に利用することができるため試用には向いていますが、有効期間が7日間という制限があるため、長期利用は推奨しません。
