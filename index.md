...
- Android版とiOS版があります
- DeployGateの仕様上、インストール可能な端末は最大100台に制限されます。
...
- **俺モニター**はGoogleへのアプリ申請を行っていないため、認証可能な端末数は最大100台に制限されます。
...
- ハングアップ
    ...
    - お気に入り画面で放置しているとハングアップしていたことがあった
- 強制終了
    - iOS版でお気に入り画面を開いたままにしていると強制終了していたことがあった
- お気に入り画面
    - iPadでスクロール中に画面が固まりやすい（メニューを切り替えたりすると直る）
        - 未調査
    - 一番下のお気に入りアイテムまで勝手にスクロールしていたことがあった
- 読み上げ
    - iOS版では音声を指定しても音声が変わらない
    - Amazon Fireタブレットでは読み上げ自体が出来ない
- チャートアイテム
    - 端末のスリープからの復帰後や取引所のメンテナンス等の再開後に、チャートアイテムが更新されなくなる場合が時々発生する
        - 調査していますが、解決するには**俺モニター**の再起動が必要になります
- ダッシュボード
    - 恐らくAndroid OSのバージョンがかなり古い端末では、Ambientへのデータの送信に失敗し「Connection failure」と通知されます
        - 未調査
    - **俺モニター**が非表示になる際、溜まっている未送信データを即時送信していますが、その時の送信に失敗しやすい（必ず失敗？）
        - 未調査
- PV余剰充電
    - EVが満充電近くになると車両が充電電力を絞るため、余剰電力が下がったと誤判断して待機モードに切り替わりやすくなります
