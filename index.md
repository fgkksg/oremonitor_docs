...
>端末に保存された記録データは、俺モニターのバックアップ機能では書き出されません。別途バックアップが必要です。

### 損失率の計算方法について
まず、次のように定義します(<u>B</u>attery, <u>C</u>harge, <u>D</u>ischarge, <u>L</u>oss)。

`B = 蓄電量の増加量（車載電池の電池残容量 1の増加量集計。減少時はマイナス）`

`C = 充電量（積算充電電力量計測値の増加量集計）`

`D = 放電量（積算放電電力量計測値の増加量集計）`

`L = 損失量（C - D - B）`

前提として次の2点を踏まえます。
- V2HのPV余剰充電機能が有効の場合は、放電モードで動作していても放電・充電のどちらも行う場合があります
- 充電量Cより少ない量がバッテリーに蓄電され、放電量Dより大きい量をバッテリーから放電しています
...
### 自動化内容
- エアコンの電源状態に応じてエアコンの以下の機器アイテムの更新周期を変えることが出来ます
...
|--|--|
|a|設定された更新周期で自動更新する|
|b|設定された更新周期で自動更新する。可能であればお気に入り画面の一番上に表示する|
|c|設定された更新周期で自動更新する。電源状態の変更時も自動更新する|
|d|設定された更新周期にかかわらず電源状態の変更時のみ自動更新する|
|e|設定された更新周期にかかわらず自動更新しない。お気に入り画面では「-」で表示する|
...
### 自動化内容
- V2H機器の運転モードに応じてV2H機器の以下の機器アイテムの更新周期を変えることが出来ます
...
- 運転モードが充電や放電の時の更新動作をa～eから選択
- 運転モードが充電でも放電でもない時の更新動作をa～eから選択

|設定|更新動作|
|--|--|
|a|設定された更新周期で自動更新する|
|b|設定された更新周期で自動更新する。可能であればお気に入り画面の一番上に表示する|
|c|設定された更新周期で自動更新する。運転モードの変更時も自動更新する|
|d|設定された更新周期にかかわらず運転モードの変更時のみ自動更新する|
|e|設定された更新周期にかかわらず自動更新しない。お気に入り画面では「-」で表示する|
