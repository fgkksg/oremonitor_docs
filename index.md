...
- Android版とiOS版がインストールできます
    - インストールにメールアドレス等は不要でアカウント作成もありません
...
### iOS版で追加となる手順
「俺モニターをインストール可能なiOS端末の情報」が俺モニター自身に書き込まれているため、他の端末では起動しません。従って、以下の手順が必要になります。
...
- 端末の機内モードがONの状態で俺モニターを起動すると、以下の状態で始まる緊急起動モードとなります
    - お気に入りアイテムの復元OFF
    - 全ての自動処理がOFF
    - スケジュール処理がOFF（日時指定で動作する機能がOFF）
    - タイトル画面が表示される
- アプリアイコンの長押しで表示されるショートカットの「緊急起動モード」メニューを使って俺モニターを起動することでも緊急起動モードで起動させることができます
- 緊急起動モード中に「設定＞バックアップ」メニューで書き出しを行えば、緊急起動モード前の状態の設定が書き出されます
- その後、機内モードOFFで俺モニターを再起動すれば、お気に入りアイテムや自動処理のON/OFF状態やスケジュール処理は元に戻ります
...
>修正されるまで、iOS版では「設定＞お気に入り＞アイテムの移動」の機能は使わないでください。
