...
# インストール
- Android版とiOS版があります
    - 現在、iOS版は致命的な不具合があります
        - **俺モニター**起動中に端末をスリープさせるとスリープを解除してもHEMS機器からのデータが受信できなくなる
...
### ユーザー登録
ユーザー登録の方法は、**俺モニター**の作者に自身のGoogleアカウント（GMailアドレス）を連絡します。
- **俺モニター**作者の連絡先メールアドレスは、認証ボタンを押して表示される画面の「OreMonitor」をクリックすると表示されます。
- 登録された後に、**俺モニター**の「OAuth認証」メニューにて認証を完了させます

### 注意点
- **俺モニター**の「設定OAuth認証」メニューにて認証プロセスを実行するとWebブラウザに切り替わりますが、ブラウザの種類によっては認証プロセスが完了しない場合があります
